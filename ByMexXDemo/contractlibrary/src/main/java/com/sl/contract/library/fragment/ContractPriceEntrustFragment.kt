...
import com.sl.contract.library.activity.ContractEntrustDetailActivity
import com.sl.contract.library.adapter.ContractPriceEntrustAdapter
 ...
 * 合约限价委托
 */
class ContractPriceEntrustFragment : BaseFragment() {
    override fun setContentView(): Int {
        return R.layout.fragment_sl_contract_hold
    }

    private var entrustAdapter: ContractPriceEntrustAdapter? = null
    ...
    private fun updateData() {
        mList.clear()
        if (isCurrentEntrust) {
            doFilterData(ContractUserDataAgent.getContractOrder(contract!!.instrument_id, true))
    ...
    private fun loadHistoryDataFromNet() {
        ...
        ContractUserDataAgent.loadContractOrder(
            contract!!.instrument_id,
            ContractOrder.ORDER_STATE_FINISH,
            mOffset,
            mLimit,
    ...
    override fun initView() {
        ...
        entrustAdapter = ContractPriceEntrustAdapter(context!!, mList)
        entrustAdapter?.setIsCurrentEntrust(isCurrentEntrust!!)
        ...
        ContractUserDataAgent.registerContractOrderWsListener(this,
            object : ContractOrderListener() {
                override fun onWsContractOrder(contractId: Int) {
                    contract ?: return
                    if (!isCurrentEntrust) {
                        return
                    }
                    SDKLogUtil.d(LogicWebSocketContract.TAG,"UserProperty合约订单更新contractId:$contractId 总条数" +ContractUserDataAgent.getContractOrder(contract!!.instrument_id).size)
                    if (contractId == 0 || contractId == contract!!.instrument_id) {
                        doFilterData(ContractUserDataAgent.getContractOrder(contract!!.instrument_id))
                        entrustAdapter?.notifyDataSetChanged()
                    }
                }
            })
            ...
            override fun onItemChildClick(
                ...
                when (view.id) {
                    ...
                    R.id.ll_item_warp_layout -> {
                        ContractEntrustDetailActivity.show(mActivity!!, mList[position])
                    }
                }
            }
        })
    ...
    companion object {
        fun newInstance(isCurrentEntrust: Boolean = true): ContractPriceEntrustFragment {
            val fg = ContractPriceEntrustFragment()
            val bundle = Bundle()
